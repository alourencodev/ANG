...
TEST_CASE("Linear Interpolations", k_tag)
{
	...
	{
	    REQUIRE(math::lerp(1, 2, 0.5f) == 1.5f);
		REQUIRE(math::lerp(0, 10, 0.75) == 7.5);
		REQUIRE(math::lerp(0, 10, 1) == 10.0f);
		REQUIRE(math::lerp(0, 10, 0) == 0.0f);
	}

	SECTION("Linear Extrapolation")
	{
	    REQUIRE(math::lerp(0, 10, 2) == 20.0f);
	    REQUIRE(math::lerp(0, 10, -1) == -10.f);
	}
}
...
{
    REQUIRE(math::nextPow2(0) == 2);		// n=0 edge case
    REQUIRE(math::nextPow2(1) == 2);
    REQUIRE(math::nextPow2(2) == 2);		// Does it work 2 itself
    REQUIRE(math::nextPow2(3) == 4);
    REQUIRE(math::nextPow2(6) == 8);
    REQUIRE(math::nextPow2(16) == 16);	// Does it work in a power of 2 different that 2
    REQUIRE(math::nextPow2(25) == 32);
}
