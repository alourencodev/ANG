...
TEST_CASE("BitField Test", k_tag)
{
	...
	SECTION("Bit manipulation")
	{
		// Set to 0000 0010
		bitfield.set(1);
		REQUIRE(bitfield == static_cast<byte>(0x02));

		// Set to 0000 0110
		bitfield.set(2);
		REQUIRE(bitfield == static_cast<byte>(0x06));
		...
		REQUIRE(bitfield == static_cast<byte>(0x04));

		// Set to 0000 0001
		bitfield.toggle(2);
		bitfield.toggle(0);
		REQUIRE(bitfield == static_cast<byte>(0x01));
	}
}
